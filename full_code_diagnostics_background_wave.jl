...
ui(x, z) = ns*Random.randn()+0.00001*sin(((1-γ)/θ*z+x)*2*pi/hu)*heaviside(x,hu-z)
vi(x, z) = ns*Random.randn()+0.00001*sin(((1-γ)/θ*z+x)*2*pi/hu)*heaviside(x,hu-z)
wi(x, z) = 10^(-2)*(ns*Random.randn()+0.0001*sin(((1-γ)/θ*z+x)*2*pi/hu)*heaviside(x,hu-z))
# bi(x,z) = 10^(-1)*ns*Random.randn()

# set simulation and decide run time
set!(model, u=ui, v=vi, w=wi) #,w = wi
...
Flow_Fields_file_name = "flow_fields_height_"*string(hu)*"_theta_"*string(θ)*"_stratification_"*string(N²)*"_interior_velocity_"*string(V∞)*"_delta_"*string(δ)*"_bo_0_visc_"*string(ν1)*"_background_wave.nc"
...
TKE_file_name = "TKE_terms_height_"*string(hu)*"_theta_"*string(θ)*"_stratification_"*string(N²)*"_interior_velocity_"*string(V∞)*"_delta_"*string(δ)*"_bo_0_visc_"*string(ν1)*"_background_wave.nc"
