...
drag_u(x, t, u, v, p) = - p.cᴰ * √((u+u_adjustment(x, 0, t, p))^2 + (v + v_pert(x,0,t,p)*p.H)^2) * (u+u_adjustment(x, 0, t, p))
drag_v(x, t, u, v, p) = - p.cᴰ * √((u+u_adjustment(x, 0, t, p))^2 + (v + v_pert(x,0,t,p)*p.H)^2) * (v + v_pert(x,0,t,p)*p.H)
