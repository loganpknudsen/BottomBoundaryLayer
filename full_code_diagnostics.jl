...
simulation.output_writers[:fields] = NetCDFOutputWriter(model, output;
                                                          ...
                                                          filename = path_name*"flow_fields_height_"*str(hu)*"_theta_"*str(θ)*"_stratification_"*str(N²)*"_interior_velocity_"*str(V∞)*".nc",
                                                          overwrite_existing = true)

simulation.output_writers[:diagnostics] = NetCDFOutputWriter(model, output2;
                                                          schedule = TimeInterval(0.005*(2*pi)/f),
                                                          filename = path_name*"TKE_terms_height_"*str(hu)*"_theta_"*str(θ)*"_stratification_"*str(N²)*"_interior_velocity_"*str(V∞)*".nc",
                                                          overwrite_existing = true)
