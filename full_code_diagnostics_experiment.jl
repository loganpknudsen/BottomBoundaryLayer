...
const δ = 0.1
const γ = ((3-S∞)/(3*(1+S∞)-4*δ*S∞) + (1+(1-δ)*S∞)^(-1))/2 # 0 PV parameter
...
# time step wizard
wizard = TimeStepWizard(cfl=0.95, max_change=1.1seconds, max_Δt=25.0seconds, min_Δt=0.001seconds) 
...
Flow_Fields_file_name = "flow_fields_height_"*string(hu)*"_theta_"*string(θ)*"_stratification_"*string(N²)*"_interior_velocity_"*string(V∞)*"_delta_"*string(δ)*"_bo_0_visc_"*string(ν1)*"_reference_acceleration.nc"
...
TKE_file_name = "TKE_terms_height_"*string(hu)*"_theta_"*string(θ)*"_stratification_"*string(N²)*"_interior_velocity_"*string(V∞)*"_delta_"*string(δ)*"_bo_0_visc_"*string(ν1)*"_reference_acceleration.nc"
