...
tau_p1 = dist.Field(name="tau_p1") # pressure gauge
...
# Problem
problem = d3.EVP([u,uz,uzz,v,vz,vzz,w,wz,wzz,b,bz,bzz,p,tau_1,tau_2,tau_3,tau_4,tau_5,tau_6,tau_7,tau_8,tau_p,tau_p1], eigenvalue=omega, namespace=locals()) # 
...
problem.add_equation("n**2*dt(w)+n**2*delta*u_sz*one_z*dx(w)+n*np.sin(theta)*v+Ri*dz(p)+lift(tau_p)+lift(tau_p1)-Ri*b*np.cos(theta)-n**(2)*Ek*wzz+lift(tau_6)=0") 
