...
f = 10**(-4)
S2 = N_list[0]**2*np.tan(theta)**2/f**2
gm = (1+S2)**(-1)
H = 1 
...
# Substitutions
z = basis.global_grid(dist,scale=1) #dist.local_grid(basis)
...
# Problem
problem = d3.EVP([u,v,w,b,p,tau_1,tau_2,tau_3,tau_4], eigenvalue=omega, namespace=locals(),tolerance=1e-10) # 
...
time = np.linspace(0,(2*np.pi)*gm**(0.5),24) #np.arange(0,(2*np.pi+1)/(1+N_list[0]**2*theta**2*f**(-2))**(0.5),1*(1+N_list[0]**2*theta**2*f**(-2))**(-0.5)) # np.arange(0,2*np.pi,0.1)
us = []
usc = []
vs = []
vsc = []
