...
WSPu = (u*w)*(u_pert(0,0,simulation.model.clock.time,p))*heaviside(hu-zC)  # AGSP contribution 
WSPv = (v*w)*(v_pert(0,0,simulation.model.clock.time,p))*heaviside(hu-zC)
...
GSP = -1*(v*w)*(γ*(θ * N²)/(f))*heaviside(hu-zC) # geostrophic shear production
